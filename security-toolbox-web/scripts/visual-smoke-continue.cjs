...
// Non-fatal wrapper to collect all Fluent/visual assertions in one run.
const fatalMethods = ["deepEqual", "deepStrictEqual", "doesNotMatch", "doesNotReject", "doesNotThrow", "equal", "match", "notDeepEqual", "notEqual", "notMatch", "notStrictEqual", "ok", "strictEqual", "throws"];
for (const name of fatalMethods) {
  const original = assert[name];
  if (typeof original !== "function") continue;
  assert[name] = function (...args) {
    try {
      original.apply(assert, args);
    } catch (err) {
...
async function verifyDashboardComposer(browser) {
  ...
  for (const viewport of viewports) {
    ...
    if (!threadBoxes[1]) {
      console.error(`[SKIP] ${size} 对话输入区结构已变，跳过该组断言`);
    } else {
